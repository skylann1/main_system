...
class dashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $members = $this->memberRepository->query()->orderBy("created_at", "DESC")->get(array("image", "name", "occupation"))->take(5);
        $alumnis = DB::table("alumni")->select(array(DB::raw("count(*) AS total_alumni , periode")))->groupBy("periode")->get();

        $meets = $this->meetRepository->query()->orderBy("created_at", "ASC")->take(5)->get(array("banner", "meetName"));
        ...
        $histories = DetailFinance::query()->orderBy("detail_finance.created_at", "DESC")->join("finance", "finance.id", "=", "detail_finance.finance_id")->where("member_id", "=", auth()->user()->member->id)->take(5)->get(array("finance.month", "detail_finance.cash", "detail_finance.paymentMethod", "detail_finance.status"));
        ...
        // return view("pages.dashboard.index", compact("title", "event", "meet", "member", "document", "balanceSheets", "members", "alumnis", "income", "outcome", "memberPaid", "meets", "documents"));
    }
}
