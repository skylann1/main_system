...
class memberEventAbsenceDataTable extends DataTable
{
    ...
    {
        return $model->query()->join("event", "event.id", "=", "event_absence.event_id")->where("event_absence.email", "=", auth()->user()->email)->where("event_absence.status", "=", "hadir")->where("event_absence.nim", "=", auth()->user()->member->nim)->select("event.id", "eventName", "event_absence.created_at", "event_absence.status", "event_absence.id as absence_id");
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make("created_at")->title("Waktu Absensi")->orderable(false)->searchable(false),
        ];
    }
